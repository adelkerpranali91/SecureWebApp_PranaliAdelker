...
app.config.update(
    SESSION_COOKIE_HTTPONLY=True, # protect session hijacking
    SESSION_COOKIE_SAMESITE="Lax", #stop cookies from cross site request
...
def register():
    if request.method == "POST": 
        ...
        if not is_strong_password(password):
            flash("Password - least 8 characters with upper, lower, number, and symbol.")
            return redirect(url_for("register"))
