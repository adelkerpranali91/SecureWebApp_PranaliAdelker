...
def init_db():
    conn = get_conn()
    cur = conn.cursor()
    # users table (insecure: plaintext passwords) - updated schema (no username column)
    cur.execute("""
...
# --- user helpers (insecure: raw SQL + plaintext password) ---
def create_user(email: str, password: str, role: str = "patient",
    ...
    conn = get_conn()
    cur = conn.cursor()
    # intentionally insecure: raw f-string SQL
    sql = (
